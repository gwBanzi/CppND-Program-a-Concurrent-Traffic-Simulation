...
T MessageQueue<T>::receive()
{
    ...
    auto r_value = std::move(_queue.back());
    _queue.clear();
    return r_value;
}
...
void TrafficLight::waitForGreen()
{
    ...
    while ( true ){
      ...
      auto current_phase = _msg.receive();
...
void TrafficLight::cycleThroughPhases()
{
    ...
    std::default_random_engine generator( start );
    std::uniform_int_distribution<int> dist(4,6);
    const auto duration = dist( generator );

    auto start_of_cycle = std::chrono::steady_clock::now();
    while ( true ) {

      const auto t_current = std::chrono::steady_clock::now();
      const auto t_elapsed = std::chrono::duration_cast<std::chrono::seconds> (t_current - start_of_cycle).count();
